...
- [x] **Enable weights** — **Done**. `MOTIF_SCORE_MULTIPLIER` set to 1.0 for both baseline and MR. Full catalog built from 11,501 symbols (34,890 keys, 6,114 passing threshold). Curve scores are small nudges (0.04-0.14) relative to total scores (10-20), adding gentle signal without dominating. Baseline backtest unchanged at 64.58% / +1.15% avg PnL (saved scores pre-date curve integration). Live validation confirms curve features flowing through pipeline.
