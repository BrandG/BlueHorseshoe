...
1. **✅ DONE 2026-06-02 — V2 order flow measured after the retirement cleared (positive result).** Forex reopened ~Sun 21:00 UTC; the 7 rising_3bar longs flattened (all reconciled to `bh_ftmo_outcomes.csv`); journal is now pure-V2. The two chokepoints the retirement targeted both cleared decisively. Comparing the prior mixed window (05-28→31) to the V2-only window (06-01+): `skip_direction_imbalance` **64 → 0** (the #1 blocker, eliminated), `skip_already_open` **47 → 5**, `skip_conflict` **15 → 0**. `order_placed` went from 8 (only **1** V2) to **6, all V2** on the first full reopened day. The net-direction gate was the limiter and it's gone with rising_3bar's long-only flood. Memory: `project_v2_order_flow_post_retirement`.
