...
- **RSI seed-init mismatch with TA-Lib** (added 2026-04-27, low priority) — momentum validation suite (commit `5e962d8`) confirmed `rsi()` converges to `talib.RSI` in steady state, but warmup divergence is significant: at `period * 5` bars the max divergence was 0.329 RSI points; needed `period * 12` (~28 days of 4h bars) before divergence dropped below 1e-3. Root cause: TA-Lib seeds with SMA-of-first-period gains/losses then switches to Wilder smoothing; bh_ftmo's `pandas.Series.ewm(alpha=1/period)` skips the SMA seed. Math is correct in steady state, but the first ~28 days of any cold-start backtest use noticeably-off RSI. Two paths: (a) leave as-is — walk-forward IS windows are 18 months so warmup is well-amortized, and incremental updates carry state in production; (b) port TA-Lib's seed convention into `rsi()` for canonical agreement. **Recommendation:** confirm walk-forward folds give signals enough warmup before generating trades, then close as wontfix unless production cold-starts hit it. The same seed pattern likely affects MACD signal-line and ADX — re-evaluate after `test_trend.py` lands.
