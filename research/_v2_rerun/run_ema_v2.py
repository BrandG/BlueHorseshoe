"""EMA distance-band v1 (first run) under v2 methodology — mirror of SMA v2."""
...
from bh_ftmo.indicators import atr, ema, ohlc_mid
...
def find_fresh_long(close, ema_arr, atr_arr, k):
    valid = ~np.isnan(close) & ~np.isnan(ema_arr) & ~np.isnan(atr_arr)
    lower = ema_arr - k * atr_arr
...
def find_fresh_short(close, ema_arr, atr_arr, k):
    valid = ~np.isnan(close) & ~np.isnan(ema_arr) & ~np.isnan(atr_arr)
    upper = ema_arr + k * atr_arr
    ...
    for period in PERIODS:
        ...
        for direction in DIRECTIONS:
            ...
            for k in K_VALUES:
                ...
                s.update({
                    "pair": pair, "period": period, "k": k, "atr_period": ATR_PERIOD,
...
def spread_test_pair(pair, period, k_val, direction, entry_mode="mid"):
    store = FxStore()
    raw = store.load(pair, granularity=GRANULARITY, include_incomplete=False)
    if raw is None or raw.empty:
        return None
    ...
    if direction == "long":
        triggers = find_fresh_long(m_close, ema_arr, atr_arr, float(k_val))
    else:
        triggers = find_fresh_short(m_close, ema_arr, atr_arr, float(k_val))
    rs = []
    for i in triggers:
        if entry_mode == "limit":
            if direction == "long":
    ...
    s = expectancy_split(rs)
    s.update({
        "pair": pair, "period": period, "k": k_val, "atr_period": ATR_PERIOD,
        "direction": direction, "total_n": len(rs),
    })
    return s


def collect_trades(pair, period, k_val, direction, entry_mode="mid"):
    store = FxStore()
    raw = store.load(pair, granularity=GRANULARITY, include_incomplete=False)
    if raw is None or raw.empty:
        return []
...
def main():
    ...
    out_dir = "/root/BlueHorseshoe/research/_v2_rerun/ema"
    os.makedirs(out_dir, exist_ok=True)
    print(f"=== EMA-band v2 walk-forward (entry={entry_mode}) ===\n", flush=True)
    all_rows = []
    t0 = time.time()
    ...
    print(f"\n=== EMA-band v2 spread test (entry={entry_mode}) ===", flush=True)
    spread_rows = []
    for idx, (_, r) in enumerate(survivors.iterrows(), 1):
        out = spread_test_pair(r["pair"], int(r["period"]), float(r["k"]), r["direction"],
    ...
    selected = select_production_cells(robust, ["pair", "period", "k", "direction"])
    for s in selected:
        print(f"  {s['pair']:<10} p={int(s['period'])} k={s['k']:.1f} "
              f"{s['direction']:<5}  te n={int(s['te_n'])} mean_R={s['te_mean_r']:+.3f}",
              flush=True)

    print("\n=== Building portfolio ===", flush=True)
    all_trades = []
    for s in selected:
        trades = collect_trades(s["pair"], int(s["period"]), float(s["k"]), s["direction"],
                                entry_mode=entry_mode)
