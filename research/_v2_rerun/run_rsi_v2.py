"""RSI v1 rerun under v2 methodology."""
        ...
        for direction in DIRECTIONS:
            ...
            for thr in THRESHOLDS:
                for rec in RECOVERY_BARS:
                    triggers = find(rsi_arr, float(thr), int(rec))
                    rs = []
                    for i in triggers:
...
def main():
    ...
    out_dir = "/root/BlueHorseshoe/research/_v2_rerun/rsi"
    os.makedirs(out_dir, exist_ok=True)
    print(f"=== RSI v2 walk-forward (entry={entry_mode}) ===\n", flush=True)
    all_rows = []
    t0 = time.time()
    ...
    print(f"\n=== RSI v2 spread test (entry={entry_mode}) ===", flush=True)
    spread_rows = []
