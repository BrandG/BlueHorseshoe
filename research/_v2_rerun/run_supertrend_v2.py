"""SuperTrend v1 (first run) under v2 methodology.

Trigger semantics: fresh direction flip.
  - Long: direction[i] == +1 AND direction[i-1] == -1
  - Short: direction[i] == -1 AND direction[i-1] == +1

This is a TREND-FOLLOWING signal — opposite shape from the band/oscillator
mean-reversion strategies. Comparable to Donchian (which produced zero
production cells). Worth testing because flip events are structurally
different from breakouts.

Param grid: ATR period, multiplier.
"""
...
from bh_ftmo.indicators import ohlc_mid, supertrend
...
def find_fresh_long(direction_arr):
    """Long: direction flipped from -1 to +1 this bar."""
    n = len(direction_arr)
    fresh = np.zeros(n, dtype=bool)
    fresh[1:] = (direction_arr[1:] == 1) & (direction_arr[:-1] == -1)
    return np.where(fresh)[0]


def find_fresh_short(direction_arr):
    """Short: direction flipped from +1 to -1 this bar."""
    n = len(direction_arr)
    fresh = np.zeros(n, dtype=bool)
    fresh[1:] = (direction_arr[1:] == -1) & (direction_arr[:-1] == 1)
    return np.where(fresh)[0]
...
def spread_test_pair(pair, period, multiplier, direction, entry_mode="mid"):
    store = FxStore()
    raw = store.load(pair, granularity=GRANULARITY, include_incomplete=False)
    if raw is None or raw.empty:
        return None
    mid = ohlc_mid(raw)
    st = supertrend(mid, period=period, multiplier=multiplier)
    dir_arr = st["direction"].to_numpy(dtype=np.int8)
    ts = raw["timestamp"].to_numpy()
    ...
    if direction == "long":
        triggers = find_fresh_long(dir_arr)
    else:
        triggers = find_fresh_short(dir_arr)
    rs = []
    for i in triggers:
        if entry_mode == "limit":
            if direction == "long":
    ...
    s = expectancy_split(rs)
    s.update({
        "pair": pair, "period": period, "multiplier": multiplier,
        "direction": direction, "total_n": len(rs),
    })
    return s


def collect_trades(pair, period, multiplier, direction, entry_mode="mid"):
    store = FxStore()
    raw = store.load(pair, granularity=GRANULARITY, include_incomplete=False)
    if raw is None or raw.empty:
        return []
...
def main():
    ...
    print(f"=== SuperTrend v2 walk-forward (entry={entry_mode}) ===\n", flush=True)
    all_rows = []
    t0 = time.time()
    ...
    print(f"\n=== SuperTrend v2 spread test (entry={entry_mode}) ===", flush=True)
    spread_rows = []
    for idx, (_, r) in enumerate(survivors.iterrows(), 1):
        out = spread_test_pair(r["pair"], int(r["period"]), float(r["multiplier"]), r["direction"],
    ...
    if robust.empty:
        print("\n*** SuperTrend NULL — zero production cells. ***", flush=True)
        return

    print("\n=== Production cell selection ===", flush=True)
    selected = select_production_cells(robust, ["pair", "period", "multiplier", "direction"])
    for s in selected:
        print(f"  {s['pair']:<10} p={int(s['period'])} m={s['multiplier']:.1f} "
              f"{s['direction']:<5}  te n={int(s['te_n'])} mean_R={s['te_mean_r']:+.3f}", flush=True)

    print("\n=== Building portfolio ===", flush=True)
    all_trades = []
    for s in selected:
        trades = collect_trades(s["pair"], int(s["period"]), float(s["multiplier"]), s["direction"],
                                entry_mode=entry_mode)
