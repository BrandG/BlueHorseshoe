"""Williams %R v1 (first run) under v2 methodology.

Parallels Stoch v2: same param grid, inverted range. %R lives in [-100, 0]:
  - Long oversold: %R near -100 (e.g., < -80, < -85)
  - Short overbought: %R near 0 (e.g., > -20, > -15)

Threshold parameter is "distance from the extreme":
  - Long base condition: %R[i-recovery] < -100 + threshold, with threshold ∈ {15, 20, 25, 30}
  - Short base condition: %R[i-recovery] > -threshold

This matches Stoch's threshold semantics so the two indicators can be compared apples-to-apples.
"""
...
from bh_ftmo.indicators import ohlc_mid, williams_r


PERIODS = [5, 9, 14, 21]
THRESHOLDS = [15, 20, 25, 30]  # distance from extreme
RECOVERY_BARS = [1, 2, 3, 4]
DIRECTIONS = ["long", "short"]


def find_fresh_long(wr_arr, threshold, recovery):
    """Long: %R rose `recovery` bars from below (-100 + threshold)."""
    n = len(wr_arr)
    if n < recovery + 1:
        return np.array([], dtype=int)
    diffs_pos = np.zeros(n, dtype=bool)
    diffs_pos[1:] = wr_arr[1:] > wr_arr[:-1]
    rising = np.zeros(n, dtype=bool)
    for i in range(recovery, n):
        rising[i] = bool(np.all(diffs_pos[i - recovery + 1: i + 1]))
    base = np.full(n, np.nan)
    base[recovery:] = wr_arr[: n - recovery]
    valid = ~np.isnan(base) & ~np.isnan(wr_arr)
    cond = valid & rising & (base < (-100.0 + threshold))
    fresh = cond & ~np.roll(cond, 1)
    fresh[0] = False
    return np.where(fresh)[0]


def find_fresh_short(wr_arr, threshold, recovery):
    """Short: %R fell `recovery` bars from above -threshold."""
    n = len(wr_arr)
    if n < recovery + 1:
        return np.array([], dtype=int)
    diffs_neg = np.zeros(n, dtype=bool)
    diffs_neg[1:] = wr_arr[1:] < wr_arr[:-1]
...
def walkforward_pair(pair, entry_mode="mid"):
    ...
    for period in PERIODS:
        wr_arr = williams_r(mid, period=period).to_numpy(dtype=float)
        for direction in DIRECTIONS:
            ...
            for thr in THRESHOLDS:
                for rec in RECOVERY_BARS:
                    triggers = find(wr_arr, float(thr), int(rec))
                    rs = []
                    for i in triggers:
...
def spread_test_pair(pair, period, threshold, recovery, direction, entry_mode="mid"):
    ...
    if direction == "long":
        triggers = find_fresh_long(wr_arr, float(threshold), int(recovery))
    else:
        triggers = find_fresh_short(wr_arr, float(threshold), int(recovery))
    rs = []
    for i in triggers:
        if entry_mode == "limit":
            if direction == "long":
...
def main():
    ...
    out_dir = "/root/BlueHorseshoe/research/_v2_rerun/wr"
    os.makedirs(out_dir, exist_ok=True)
    print(f"=== Williams %R v2 walk-forward (entry={entry_mode}) ===\n", flush=True)
    all_rows = []
    t0 = time.time()
    ...
    print(f"\n=== Williams %R v2 spread test (entry={entry_mode}) ===", flush=True)
    spread_rows = []
    ...
    if spread_df.empty:
        print("\n*** No spread-test rows — early return. ***", flush=True)
        return
