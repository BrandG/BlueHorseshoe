"""ADX parameter sweep — EXTENDED past the boundary to pin the true ceiling.

Same honest yardstick: ATR 2:1 runner (TP=2*ATR14, SL=1*ATR14), entry close[t],
stop-first on same-bar tie, timeout = mtm in R. Objective = expectancy LIFT vs the
random-date baseline (same regime/window). Windows 10 & 15.

Extended knobs (prior optimum sat on the grid edge at period=28, adx_min=30):
  period   : 21, 28, 35, 50, 75, 100
  adx_min  : 30, 35, 40, 45
  adx_max  : 60, 999(none)
  rising   : 0/1
  di       : off | s0(+DI>-DI) | s20((+DI--DI)>20)
288 configs. Prints a PERIOD-RESPONSE CURVE (best lift per period) + top configs.
"""
...
PERIODS=[21,28,35,50,75,100]; ADX_MIN=[30,35,40,45]; ADX_MAX=[60,999]; RISING=[0,1]; DI=["off","s0","s20"]
...
df=pd.DataFrame(rows); df.to_csv("research/indicator_screen/adx_param_sweep_ext.csv",index=False)

for rk in ("nonbull","all","bull"):
    base=stat(("__BASE__",),10,rk)
    print(f"\n############ regime={rk}  (N=10 baseline exp={base['exp']:+.4f}R) ############")
    sub=df[(df.regime==rk)&(df.N==10)&(df.n>=2000)&(df.t>=2)]
    print("  -- PERIOD RESPONSE: best lift achievable at each period (n>=2000,t>=2) --")
    for p in PERIODS:
        s2=sub[sub.period==p]
        if s2.empty: print(f"    period {p:3}: (no clear config)"); continue
        r=s2.loc[s2.lift.idxmax()]
        print(f"    period {p:3}: best lift={r.lift:+.4f}R (adx_min={int(r.adx_min)} cap={int(r.adx_max)} rise={int(r.rising)} di={r.di} | t={r.t:+.1f} exp={r.exp:+.4f}R n={int(r.n):,})")
    print("  -- ADX_MIN RESPONSE: best lift at each threshold --")
    for am in ADX_MIN:
        s2=sub[sub.adx_min==am]
        if s2.empty: print(f"    adx_min {am}: (none)"); continue
        r=s2.loc[s2.lift.idxmax()]
        print(f"    adx_min {am}: best lift={r.lift:+.4f}R (period={int(r.period)} di={r.di} | t={r.t:+.1f} n={int(r.n):,})")
    print("  -- TOP 10 configs by expectancy lift --")
    for _,r in sub.sort_values("lift",ascending=False).head(10).iterrows():
...
print("\nsaved -> research/indicator_screen/adx_param_sweep_ext.csv")
