...
print(f"fundamentals-covered {len(covered)}  | full-book sim, {NCAL} trading days",flush=True)

def dv_cost_bp(dv): return 50.0 if dv<1e6 else 25.0 if dv<5e6 else 12.0 if dv<25e6 else 6.0

# ---- precompute all fire events across symbols ----
FIRES=[]   # dict per filled fire
for ii,sym in enumerate(covered):
    if ii%300==0: print(f"  scan {ii}/{len(covered)}",flush=True)
