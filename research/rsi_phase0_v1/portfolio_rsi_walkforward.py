"""RSI portfolio walk-forward: pick one cell per surviving pair, build ledger,
report portfolio metrics.

Mirror of research/stoch_phase0_v1/portfolio_stoch_walkforward.py.
"""
...
SURVIVING_PAIRS = ["EUR_GBP", "CHF_JPY"]
GRANULARITY = "H4"
MAX_HOLD = 14 * 6
TRAIN_FRAC = 0.7
TP_PCT = 0.01
...
SPREAD_CSV = "/root/BlueHorseshoe/research/rsi_phase0_v1/walkforward_rsi_spread.csv"
...
def collect_trades(pair, period, threshold, recovery, direction):
    ...
    if direction == "long":
        triggers = find_fresh_long(rsi_arr, float(threshold), int(recovery))
    else:
        triggers = find_fresh_short(rsi_arr, float(threshold), int(recovery))
