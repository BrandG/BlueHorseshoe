"""Walk-forward RSI sweep with REAL OANDA spread (bid/ask fills).
...
MID_CSV = "/root/BlueHorseshoe/research/rsi_phase0_v1/walkforward_rsi_triggers.csv"
...
def main():
    out_path = "/root/BlueHorseshoe/research/rsi_phase0_v1/walkforward_rsi_spread.csv"
    print(f"Walk-forward with REAL spread, output: {out_path}\n")
    ...
    print(f"Robust survivors WITH SPREAD: {len(robust)}/{len(df)}")
    ...
    if mid_path.exists():
        ...
        key_cols = ["pair", "period", "threshold", "recovery", "direction"]
        ...
        for _, r in merged.sort_values("te_wr_spr", ascending=False).iterrows():
            flag = "★★ ROBUST" if r["spr_robust"] else "✗ FAIL"
            ci_str = f"[{r['tr_ci_low_spr']*100:.1f},{r['te_ci_low_spr']*100:.1f}]"
            print(f"  {r['pair']:<8} p={int(r['period']):>2} thr={int(r['threshold']):>3} "
                  f"rec={int(r['recovery'])} {r['direction']:<5}  "
                  f"mid te WR={r['te_wr_mid']*100:.1f}%  spr te WR={r['te_wr_spr']*100:.1f}%  "
                  f"Δ={r['delta_te_wr']:+.1f}  spr CI={ci_str}  {flag}")
