"""SMA band-strategy portfolio walk-forward: pick one cell per surviving pair,
combine into a single chronological trade ledger, report portfolio metrics.

Spread-aware (uses the same fills as walkforward_sma_band_spread.py).

Selection rule (mirrors BB v1 / Stoch v1 portfolio scripts): per pair, pick the
robust cell (CI lower > 50% on both halves) with the largest te_n. Ties broken
by te_wr.
...
Portfolio metrics:
  ...
  - cross-pair monthly R correlation
"""
...
SURVIVING_PAIRS = ["EUR_CAD", "EUR_GBP", "GBP_CAD", "CAD_JPY", "AUD_CHF"]
GRANULARITY = "H4"
MAX_HOLD = 14 * 6
ATR_PERIOD = 14
TRAIN_FRAC = 0.7
...
SPREAD_CSV = "/root/BlueHorseshoe/research/sma_phase0_v1/walkforward_sma_band_spread.csv"


def find_fresh_long(close, sma_arr, atr_arr, k):
...
def find_fresh_short(close, sma_arr, atr_arr, k):
    valid = ~np.isnan(close) & ~np.isnan(sma_arr) & ~np.isnan(atr_arr)
...
def collect_trades(pair, period, k_val, direction):
    store = FxStore()
    ...
    if direction == "long":
        triggers = find_fresh_long(m_close, sma_arr, atr_arr, float(k_val))
    else:
        triggers = find_fresh_short(m_close, sma_arr, atr_arr, float(k_val))
...
def main():
    ...
    for pair in SURVIVING_PAIRS:
        ...
        for _, r in pair_cells.iterrows():
            print(f"    p={int(r['period']):<3} k={r['k']:.1f} {r['direction']:<5}  "
        ...
        print(f"    PICK: p={int(chosen['period'])} k={chosen['k']:.1f} "
              f"{chosen['direction']}\n")
    ...
    for s in selected:
        ...
        print(f"  {s['pair']:<10} {s['direction']:<5} p={int(s['period'])} "
              f"k={float(s['k']):.1f} → {len(trades)} trades")
