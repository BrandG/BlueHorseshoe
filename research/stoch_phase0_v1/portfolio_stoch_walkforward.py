"""Stochastic portfolio walk-forward: pick one cell per surviving pair, combine
into a single chronological trade ledger, and report portfolio-level metrics.

Spread-aware (uses the same fills as walkforward_stoch_spread.py).

Selection rule (mirrors BB v1 portfolio script): per pair, pick the robust cell
(CI lower > 50% on both halves) with the largest te_n. Ties broken by te_wr.
...
SURVIVING_PAIRS = ["CHF_JPY", "EUR_GBP", "USD_JPY", "CAD_CHF"]
GRANULARITY = "H4"
MAX_HOLD = 14 * 6
TRAIN_FRAC = 0.7
TP_PCT = 0.01
...
SPREAD_CSV = "/tmp/walkforward_stoch_spread.csv"
...
def collect_trades(pair, k_period, d_period, threshold, recovery, direction):
    store = FxStore()
...
def main():
    ...
    for pair in SURVIVING_PAIRS:
        ...
        for _, r in pair_cells.iterrows():
            print(f"    kP={int(r['k_period']):<2} thr={int(r['threshold'])} "
        ...
        print(f"    PICK: kP={int(chosen['k_period'])} thr={int(chosen['threshold'])} "
    ...
    for s in selected:
        ...
        print(f"  {s['pair']:<10} {s['direction']:<5} kP={int(s['k_period'])} "
    ...
    spec = {s["pair"]: {
        "k_period": int(s["k_period"]),
        "d_period": int(s["d_period"]),
