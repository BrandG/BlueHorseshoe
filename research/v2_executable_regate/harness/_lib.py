...
def sim_long_limit_spread(
    ca, hb, lb, cb, limit_signal, fill_ask_low, fill_baseline_low, i, max_hold,
    fill_window=LIMIT_FILL_WINDOW,
):
    """Spread variant of sim_long_limit. Limit price is the archived signal level.
    A buy limit fills only when the fill-window bar's ask-low reaches that level.
    Entry remains the limit price; long exits keep the archived bid-side checks.
    """
    limit_price = limit_signal[i]
    fill_idx = None
    for j in range(1, fill_window + 1):
        k = i + j
        if k >= len(ca):
            return None, None
        if fill_ask_low[k] <= limit_price:
            assert fill_baseline_low[k] <= limit_price, "executable long limit fill was not a baseline-touch fill"
            fill_idx = k
            break
    if fill_idx is None:
        return None, None
...
def sim_short_limit_spread(
    cb, ha, la, ca, limit_signal, fill_bid_high, fill_baseline_high, i, max_hold,
    fill_window=LIMIT_FILL_WINDOW,
):
    """Spread variant of sim_short_limit. Limit price is the archived signal level.
    A sell limit fills only when the fill-window bar's bid-high reaches that level.
    Entry remains the limit price; short exits keep the archived ask-side checks.
    """
    limit_price = limit_signal[i]
    fill_idx = None
    for j in range(1, fill_window + 1):
        ...
        if k >= len(cb):
            return None, None
        if fill_bid_high[k] >= limit_price:
            assert fill_baseline_high[k] >= limit_price, "executable short limit fill was not a baseline-touch fill"
            fill_idx = k
            break
    if fill_idx is None:
        return None, None
