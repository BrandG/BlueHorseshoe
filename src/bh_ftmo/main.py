"""BH FTMO: daily-bar FTMO signal generator using BlueHorseshoe scoring.

Phase 0 seed: copy of bh_lite.py with LiteTrader renamed to FTMOTrader and
file paths pointed at bh_ftmo_*.json. bh_lite.py remains frozen per decision
C-5 in docs/planning/BH_FTMO_PLAN.md.
"""
...
DEFAULT_CONFIG_PATH = os.path.join(_SRC_DIR, "bh_ftmo_config.json")
DEFAULT_POSITIONS_PATH = os.path.join(_SRC_DIR, "bh_ftmo_positions.json")


def load_config(path: str = DEFAULT_CONFIG_PATH) -> dict:
    """Load BH FTMO JSON config."""
...
class FTMOTrader:
    ...
    def _get_dynamic_atr_discount(technical_score: float) -> float:
        ...
        signal_class = FTMOTrader._classify_signal_strength(technical_score)
...
def check_position_health(position: dict, config: dict) -> Optional[dict]:
    ...
    atr = FTMOTrader()._calculate_atr(enriched)
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    parser.add_argument("--csv", action="store_true", help="Write ranked signals to src/logs/bh_ftmo_YYYY-MM-DD.csv.")
    ...
    orders_path = os.path.join(_SRC_DIR, "bh_ftmo_orders.json")
    _write_orders(ranked, orders_path)
    print(f"Orders template: {orders_path}")

    if args.csv:
        path = os.path.join("src", "logs", f"bh_ftmo_{date.today().isoformat()}.csv")
