...
def rebuild_scores(start_date: str, end_date: str):
    """
    ...
    while current <= end:
        ...
        if current.weekday() < 5:
            ...
            trader.swing_predict(target_date=date_str)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("Usage: python src/rebuild_scores.py <start_date> <end_date>")
        print("Example: python src/rebuild_scores.py 2025-11-10 2025-12-15")
        sys.exit(1)
        
    start_dt = sys.argv[1]
    end_dt = sys.argv[2]
    
    rebuild_scores(start_dt, end_dt)
    print("\nRebuild complete.")
