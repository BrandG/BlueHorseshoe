...
CLI tool to run the GradingEngine on historical trade scores stored in MongoDB.
"""
import argparse
import logging
import sys
...
def print_overall_stats(results: list):
    """Prints overall performance statistics."""
    df_results = pd.DataFrame(results)
    df_results = df_results[df_results['status'].isin(['success', 'failure'])]
    if not df_results.empty:
        overall_win_rate = (df_results['status'] == 'success').mean() * 100
        overall_pnl = df_results['pnl'].mean()
        print("\nOverall Performance:")
