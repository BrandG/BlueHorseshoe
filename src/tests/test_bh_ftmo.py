"""Tests for BH FTMO signal generator (Phase 0 seed, copied from test_bh_lite.py)."""
...
def test_fetch_ohlcv_columns(mocker):
    ...
    mocker.patch("bh_ftmo.main.requests.get", return_value=mock_resp)
...
def test_zero_volume_does_not_produce_nan():
    ...
    setup = FTMOTrader().calculate_baseline_setup(enriched, technical_score=5)
...
def test_lite_trader_baseline_setup():
    df = enrich_dataframe(make_ohlcv())
    setup = FTMOTrader().calculate_baseline_setup(df, technical_score=10)
...
def test_lite_trader_mr_setup():
    df = enrich_dataframe(make_ohlcv())
    setup = FTMOTrader().calculate_mean_reversion_setup(df)
...
def test_find_instrument_by_ftmo():
    from bh_ftmo.main import _find_instrument_by_ftmo
...
def test_position_health_ok(mocker):
    from bh_ftmo.main import check_position_health
    ...
    mocker.patch("bh_ftmo.main.fetch_ohlcv", return_value=df)
    mocker.patch("bh_ftmo.main.score_instrument", return_value={"baseline_score": 20.0, "mean_reversion_score": 12.0})
...
def test_position_health_weakening(mocker):
    ...
    position = {"ftmo_symbol": "GSPC.sim", "side": "buy", "entry": 100.0, "stop": 90.0, "lots": 2.0}
    mocker.patch("bh_ftmo.main.fetch_ohlcv", return_value=df)
    mocker.patch("bh_ftmo.main.score_instrument", return_value={"baseline_score": 3.0, "mean_reversion_score": 2.0})
...
def test_position_health_critical(mocker):
    ...
    mocker.patch("bh_ftmo.main.fetch_ohlcv", return_value=df)
    mocker.patch("bh_ftmo.main.score_instrument", return_value={"baseline_score": 0.0, "mean_reversion_score": -1.0})
...
def test_position_pnl_forex():
    from bh_ftmo.main import _calculate_position_pnl
